...
def ZeroShot_Exp(args):
    ...
    pipeline = ChronosPipeline.from_pretrained(
        args.checkpoints,
        device_map=args.which_gpu,  # use "cpu" for CPU inference and "mps" for Apple Silicon
    ...
    for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
        ...
        forecast_mid = torch.median(forecast, dim=1)[:]
        ...
        # with open('./forecast_log/{}_mid_5.jsonl'.format(args.data), 'a') as outfile:
        #     for item in forecast_mid[0]:
        #         json.dump(item.numpy(), outfile, cls = MyEncoder)
        #         outfile.write('\n')
        # with open('./forecast_log/{}_mean_5.jsonl'.format(args.data), 'a') as outfile:
        #     for item in forecast_mean:
        #         json.dump(item.numpy(), outfile, cls = MyEncoder)
        #         outfile.write('\n')
        # with open('./forecast_log/{}_original_5.jsonl'.format(args.data), 'a') as outfile:
        #     for item in forecast:
        #         json.dump(item.numpy(), outfile, cls = MyEncoder)
        #         outfile.write('\n')
        # with open('./forecast_log/{}_gt_5.jsonl'.format(args.data), 'a') as outfile:
        #     for item in batch_y:
        #         json.dump(item.numpy(), outfile, cls = MyEncoder)
        #         outfile.write('\n')
